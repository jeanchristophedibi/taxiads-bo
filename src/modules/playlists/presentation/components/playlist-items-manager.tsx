...
export function PlaylistItemsManager() {
  const { data: playlists = [], isLoading: loadingPlaylists } = useOptionsQuery('playlists');
  const [playlistId, setPlaylistId] = useState('');
  const playlistLabel = playlists.find((p) => p.key === playlistId)?.value ?? '';

  const { data, isLoading } = usePlaylistItemsQuery(playlistId || undefined);
  ...
  const onSave = () => {
    if (!playlistId || !form.page_id || !form.layout_id) return;
    ...
    <div className="space-y-4">
      ...
      {!playlistId ? (
        <div className="bg-white rounded-xl border border-slate-200 p-8 text-center text-sm text-slate-400">
          Sélectionne une playlist pour gérer ses items.
        </div>
      ) : (
        <div className="bg-white rounded-xl border border-slate-200 overflow-hidden">
          <div className="px-5 py-4 border-b border-slate-100">
            <h2 className="text-base font-semibold text-slate-900">Items de la playlist</h2>
            <p className="text-xs text-slate-500 mt-0.5">{playlistLabel}</p>
          </div>

          <div className="grid grid-cols-1 lg:grid-cols-2 gap-0">
            <div className="border-r border-slate-100 p-5 max-h-[60vh] overflow-auto">
            ...
            <div className="p-5 max-h-[60vh] overflow-auto">
              ...
              <div className="space-y-3">
                ...
                <div className="flex items-center justify-end gap-2 pt-2">
                  {form.id && <button onClick={clearForm} className="px-3 py-2 text-sm text-slate-500 hover:text-slate-700">Nouveau</button>}
                  <button
                    onClick={onSave}
                    disabled={isSaving || !playlistId || !form.page_id || !form.layout_id}
